...
from appium.webdriver.extensions import action_helpers as ac
...
class ElementActions:
    ...
    def sleep(s):
        return time.sleep(s)
    def tap(self,positions,dura):
        self.driver.tap(positions,duration=dura)
    ...
    def click(self, locator, count=1):
        ...
        if count == 1:
            ...
            try:
                ...
                pass
    def long_press(self,locator,dua):
        el = self._find_element(locator)

        touch_action = TouchAction(self.driver)
        touch_action.long_press(el,duration=dua).perform()
    ...
    def swip_up(self, count=1, method=None):
        ...
        Examples:
            action.swip_up(count=1, method=lambda action:  action.is_key_text_displayed("暂无可配送的订单"))
            上面代码意思:当页面不展示"暂无可配送的订单"时停止刷新,即有单停止刷新
        """
        if count == 1:
    ...
    def setUp(self):
        if self.is_text_displayed('Welcome to togetU'):
            self.back_press()
        else:
            pass
        """
        ...
        """
        time.sleep(2)
        if  self.is_text_displayed('Swipe up to view more videos')  :
            self.tap([(699, 900), (100, 1000)], 1500)

        else:
            pass
        self.back_press()
        """
        检查是否已登陆，未登陆进入登陆状态，登录状态的话就结束
        """
